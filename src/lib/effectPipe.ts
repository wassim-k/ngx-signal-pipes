...
export function effectPipe(...signals: Array<Signal<any>>): any {
  const aggregator = signals.length === 1
    ? signals[0]
    : () => signals.map((s: Signal<any>) => s());

  return new EffectPipeBuilder(aggregator as Signal<any>);
}

export class EffectPipeBuilder<T> {
  private readonly pipes: Array<EffectPipe<T>> = [];

  public constructor(private readonly aggregator: Signal<T>) { }
  ...
  public run(fn: EffectPipeFn<T>, options?: CreateEffectOptions): EffectRef {
    ...
    const ref = effect(
      onCleanup => pipeline(this.aggregator(), onCleanup, ref),
      options
    );
